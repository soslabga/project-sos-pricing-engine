...
OUT = "C:/tmp/coworking_general_model_v21.zip"
FINAL = "coworking_general_model_v21.xlsx"
LAYOUT_IMAGES = [
    ("100평형", "배치도_100평.png"),
    ("120평형", "배치도_120평.png"),
    ("150평급(전용145평)", "배치도_150평.png"),
]
...
SIZE_MODELS = [
    ...
    ("150평급(분당, 전용145평)", 145, 52, 106, 17, 3, 32, 123, 62, 41, 560, 350, 120, "08_부동산원자료 시트 원본 대조(2026.7.9)"),
]
...
def add_layout(wb):
    # 100/120/150평급 실제 배치도(SVG→PNG 렌더, 지역명·가격 텍스트 제거된 중립본). 방구성은 00_규모모델(분당3종) SIZE_MODELS와 정확히 일치(2026-07-10 재검증).
...
def add_common_assumptions(wb):
    ...
    ws.append(["관리비 단가(만원/평)", 1.5, "설계 가정치 — 99_작업로그 참조"])  # row3
    ws.append(["전기 단가(만원/평)", 0.6, "설계 가정치 — 99_작업로그 참조"])  # row4
    ...
    ws.append(["운전자금(만원)", 500, "설계 가정치 — 99_작업로그 참조"])  # row14
    style(ws)
...
def add_size_model(wb):
    ...
    add_row("모델 성격(비고)", ["전국 확장 표준모델(분당 기준). 월세: 분당 실매물 기준(검증). 객실 구성(룸믹스)·투자비(CAPEX·보증금): 설계 기준(가정치, 실사 전 잠정). 분당 표본 기반 가설, 전국 검증 미완료."] * 3)  # row42
    ws.append(["출처"] + [m[13] for m in SIZE_MODELS])          # row43
    ws.append(["※ 방구성·전용평수·초기투자비는 실제 배치도 기준 고정값, 지역 티어(B2) 변경 시 전용평당월세·객실가·월세만 재계산됨. 상세 검증 이력은 99_작업로그 참조."])  # row44
    ...
    notes = {
        ...
        21: "파트타임 인건비 — 150평급만 인력 추가 운영, 100·120평형은 없음",
        22: "CAPEX 단가는 설계 가정치 — 상세 근거는 99_작업로그 참조",
        ...
        41: "보증금은 설계 가정치 — 상세 근거는 99_작업로그 참조",
        42: "상세 근거는 99_작업로그 참조",
        43: "각 평형의 방 구성·면적·초기투자비 산출 근거는 99_작업로그 참조",
    }
...
def add_rent_model(wb):
    ...
    for tier, rent, p1, p2, p4, desc, decision in TIERS:
        row = ws.max_row + 1
        ws.append([tier, rent, p1, p2, p4, desc, "='99_참고자료_구버전'!B3", f"='03_매출'!B3*C{row}+'03_매출'!B4*D{row}+'03_매출'!B5*E{row}", f"=G{row}*B{row}+'99_참고자료_구버전'!B17", f"=I{row}/(H{row}*(1-'99_참고자료_구버전'!B18))", f"=H{row}*0.7*(1-'99_참고자료_구버전'!B18)-I{row}", f"=H{row}*0.9*(1-'99_참고자료_구버전'!B18)-I{row}", decision])
...
def add_capex(wb):
    ws = wb.create_sheet("99_참고자료_구버전")
...
def add_pl(wb):
    ...
    for src in range(31, 36):
        for occ in [0.5, 0.6, 0.7, 0.8, 0.9, 1.0]:
            ws.append([f"='02_임대료모델'!A{src}", occ, f"=1-B{out}", f"='02_임대료모델'!H{src}", f"=D{out}*B{out}", f"='02_임대료모델'!I{src}", f"=E{out}*'99_참고자료_구버전'!B18", f"=E{out}-F{out}-G{out}", f"=H{out}*12"])
            out += 1
    style(ws)
...
def add_investment_recovery(wb):
    ...
    add_row("모델 성격", [f"={SM}{c}42" for c in cols])  # row3 — 3개 평형 모두 분당 기준 전국 확장 표준모델 가설
    ...
    ws.append([])  # row23
    ws.append(['="결론 — 분당 3개 규모(100/120/145평) 전국 확장 표준모델 가설. 월세는 분당 실매물 기준(검증), 룸믹스·CAPEX·보증금은 설계 기준(가정치). 회수기간(총소요자금 기준): 100평형 "&TEXT(B22,"0.00")&"년, 120평형 "&TEXT(C22,"0.00")&"년, 150평급(전용145평) "&TEXT(D22,"0.00")&"년. 현재 검증 표본은 분당 지역(더분당36실·스파크플러스분당11실)에 한정됨 — 전국 검증 완료 단계 아님."'])  # row24 — 총소요자금 기준 회수기간(년)은 B22/C22/D22 실시간 계산값 그대로 표시
    ...
    notes = {
        ...
        15: "반환의무 부채(월세와 별개). 설계 가정치 — 99_작업로그 참조",
        16: "설계 가정치 — 99_작업로그 참조",
...
def add_worklog(wb):
    # 경영진용 시트의 짧은 각주("설계 가정치 — 99_작업로그 참조" 등)에 대한 상세 근거를 여기 모아 관리
    ws = wb.create_sheet("99_작업로그")
    ws.append(["작업 로그 및 상세 검증 이력 — 경영진용 시트의 요약 각주에 대한 상세 근거"])
    ws.append(["관련 시트", "항목", "상세 내용 및 정정 이력", "일자"])
    rows = [
        ("00_규모모델(분당3종)", "보증금(41행)",
         "100/120/150평급 보증금(7,500/7,500/10,000만원)의 원본 출처를 프로젝트 내 mjs 파일(standard_100.mjs, pangyo_120.mjs, pangyo_150.mjs) 및 프로젝트 폴더 전체에서 검색했으나 확인 불가. 설계 가정치로 잠정 사용 중이며 실사 재확인 필요.",
         "2026-07-10"),
        ("00_규모모델(분당3종)", "초기투자비 단가(22행)",
         "CAPEX 산식(평수×163+실수×7+좌석수×23.65+286)의 단가 출처를 재확인하는 과정에서, 관리비·전기 실측치 인용(비용_항목별_검증표.html)이 실제로는 확인 불가능한 자료였음을 발견 — 확인 불가로 재분류하고 설계 가정치로 명시.",
         "2026-07-10"),
        ("00_공통가정", "관리비·전기 단가(3~4행)",
         "관리비 1.5만/평, 전기 0.6만/평의 근거로 '실측 판교관리비고지서(0.75만/평)'를 인용했으나 원본 확인 불가로 판명 — 출처 인용 삭제, 설계 가정치로 명시.",
         "2026-07-10"),
        ("00_공통가정", "운전자금(14행)",
         "운전자금 500만원의 근거로 pangyo_120.mjs/pangyo_150.mjs의 총소요자금 계산식(CAPEX+보증금+운전자금)을 인용했으나, 이는 계산 로직 참조일 뿐 500만원이라는 금액 자체의 별도 출처는 아님 — 설계 가정치로 명시.",
         "2026-07-10"),
        ("99_참고자료_구버전", "시트 성격",
         "00_규모모델(분당3종) 도입 이전 100평 단일모델 체계. 관리비 단가 30,000원/평으로 현재 00_공통가정(15,000원/평)과 불일치. 02_임대료모델 TIERS 표·05_손익이 여전히 이 시트를 참조하나 경영진 보고 범위 밖 — 시트명을 04_초기투자비(구버전_미사용)에서 99_참고자료_구버전으로 변경하고 워크북 말미로 이동.",
         "2026-07-10"),
        ("06_영업인력", "BEP·90%영업이익(F·G열)",
         "구버전 02_임대료모델 티어(D행 고정) 참조하던 BEP 산식을 00_규모모델(분당3종) 최신값으로 교체하는 과정에서, E열(지점당 배부비)이 원 단위인데 00_규모모델!B24(만원 단위)에 그대로 더해지는 단위불일치 발생 — BEP 688% 등 비정상값 산출. E열에 /10000 보정 추가로 수정, 검산 완료(BEP 67.6~74.5% 범위 확인).",
         "2026-07-10"),
        ("07_경쟁사분석", "리더스 분당",
         "글로벌 리저스(Regus) 브랜드와 연관된 것으로 잘못 표기됐던 '리더스 분당(Regus)'을 '리더스 분당'으로 정정 — 글로벌 리저스와 무관한 별도 국내 로컬 업체로 확인됨.",
         "2026-07-10"),
        ("07_경쟁사분석", "패스트파이브 판교 4인실",
         "실거래 할인가(−28%, 2,224,800원)로 추정했던 수치가 실제로는 존재하지 않는 할인이었음이 확인됨 — 정가(3,090,000원) 기준으로 전면 정정.",
         "2026-07-10"),
        ("00_규모모델(분당3종)", "150평급 전용평수·월세",
         "판교 파일럿 모델(전용150평)에서 분당 실매물 기준(전용145평, '분당 큰평수.png' 전용480㎡)으로 정정. 120평형 월세도 435만원으로 실매물('분당100평 수정.png' 전용398㎡) 대조 반영. 3개 평형 모두 판교가 아닌 분당 기준으로 통일.",
         "2026-07-10"),
        ("10_투자회수", "회수기간 계산 표본",
         "현재 회수기간 계산은 분당 지역 임대료 시세(02_임대료모델 지역 티어)와 분당 두 실매물의 공실 데이터(더분당 36실, 스파크플러스 분당 11실)를 기반으로 한 가설이며, 마곡·구로디지털단지 등 타 일반업무권역 표본은 미확보 — 전국 검증 완료 단계 아님.",
         "2026-07-10"),
    ]
    for sheet, item, detail, date in rows:
        ws.append([sheet, item, detail, date])
    style(ws)
