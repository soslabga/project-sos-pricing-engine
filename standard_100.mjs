...
s.push(`<text x="160" y="60" font-size="11" fill="#64748b">줄1: 4인 3,670×2,400(풀폭·가로형) · 줄2~3: 4인 3,440×2,900 · 줄4: 2인 1,840×3,600(2.0평) · 1인 2,080×1,750(1.1평) · 복도1,200</text>`);
...
// 줄4: 2인 4실(1,840×3,600=2.0평, SP1.75<우리<로컬2.1) + 1인 8실(2,080×1,750=1.1평, 4열×2행) — FF/SP보다 넓게·로컬 이하. 호실수 동일(좌석 72 유지)
x=0.12;
for(let i=0;i<4;i++){room(x,11.02,1.84,3.6,2,'#dce8ff','1840×3600','top');x+=2.02;}
for(let i=0;i<4;i++){
  room(x,11.02,2.08,1.75,1,'#ede8ff','2080×1750','top');
  room(x,12.87,2.08,1.75,1,'#ede8ff','2080×1750','bottom');
  x+=2.27;
}
...
const py=105, rent=440, depo=7500, mgmtRate=1.5, partTime=0, labor=350; // 관리비 1.5만/평=실측판교관리비고지서(0.75만/평) 보수적 중간값 // 현장담당 1인운영 인건비(지점손익 기준). 유사직무(오피스텔 시설관리) 연봉3600~3800만=월300~317만 기본급 + 4대보험11%+퇴직충당금8.3%(법정부담 약20%) 웹검증. 마케팅7%는 CONT 0.885에 이미 반영
// [가격정책 확정 2026-07-01] 원가+25%마진 cost-plus 전국표준 산식(pangyo11.mjs와 동일 방법론)
const full_rev=mix[4]*134+mix[2]*87+mix[1]*30;
...
console.log(`[런칭기 참고, 1~6개월] 정액 마케팅비 +${launchMkt}만 가정 시: 고정비${fk(fixedLaunch)}만 · BEP ${bepLaunch.toFixed(1)}% (정상 BEP 아님, 초기 한시적. 이후 매출7% 변동비로 자연 감소)`);
console.log('');
console.log('=== 분당 3개 모델 BEP 비교 ===');
console.log(`100평: ${rm}실/${seat}석 · BEP ${bep.toFixed(1)}%`);
console.log('120평: 33실/90석 · BEP 65.8% (최신 실행 결과)');
console.log('150평: 53실/110석 · BEP 65.3% (최신 실행 결과)');
