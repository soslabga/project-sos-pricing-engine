...
// 가격: 120/150평과 동일(133/78/47) — 이전 +9% 프리미엄은 "BEP를 특정 수치에 맞추기 위한 역산값"으로 근거가 없어 제거함
...
const capex=Math.round((py*150+py*7+py*7.61+py*0.43+1999.25+rm*7+200+seat*23.65+50+100)*1.05); // 냉난방7.61만/평+소방0.43만/평(이미지 OCR 실제값: 냉난방 약2,293.6만, 소방 128.6만, 판교4층 301.54평 기준) // 무선AP 1,999.25만 추가 + 라운지가구(아웃도어풍) 100만 정액 추가 // 전기통신11→7만/평 확정검증(평촌임대차계약서 37평×전기및네트워크.png 247.5만원=6.69만/평, 판교 통신전용4.05만/평과 역산 정합) + 도어락7만/룸 + 보안200만
...
console.log('120평: 33실/90석 · BEP 57.8% (전체 이미지 실제값 반영 최신 실행 결과)');
console.log('150평: 53실/110석 · BEP 56.7% (전체 이미지 실제값 반영 최신 실행 결과)');
