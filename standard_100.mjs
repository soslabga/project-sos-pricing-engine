...
const capex=Math.round((py*150+py*11+rm*7+200+seat*23.65+50)*1.05); // 전기통신11만/평 유지(통신 몫만 4.05만/평 검증됨-4층계약서301.54평×네트워크공사1,221만원, 네트워크공사2.png. 전기/콘센트/분전반 몫은 별도공사라 미검증, 11만 그대로) + 도어락7만/룸 + 보안200만
const dep=Math.round(capex/60);
const mgmt=Math.round(py*mgmtRate), elec=Math.round(py*0.6); // 실측 판교1103호 관리비고지서 반영(보수적 중간값)
const clean=Math.round(py*0.7), opexFlat=32; // 인터넷4+복합기10+정수기3+CCTV보안12(ADT캡스 실제 월정료124,400원 기준, 캡스견적서2.png)+화재보험3
...
console.log('120평: 33실/90석 · BEP 54.3% (참고: pangyo_120.mjs 최신 실행 결과)');
console.log('150평: 53실/110석 · BEP 53.6% (참고: pangyo_150.mjs 최신 실행 결과)');
