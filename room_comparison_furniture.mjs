...
const desk=(x,y,w,h,ox,oy)=>`<g><rect x="${ox+x*S}" y="${oy+y*S}" width="${w*S}" height="${h*S}" fill="#d9c9a3" stroke="#8a6d3b" stroke-width="2" rx="3"/><text x="${ox+(x+w/2)*S}" y="${oy+(y+h/2)*S+4}" font-size="9.5" fill="#5c4a2a" text-anchor="middle">1200×700</text></g>`;
...
s.push(`<text x="${totalW/2}" y="40" font-size="20" font-weight="800" fill="#0f1e3d" text-anchor="middle">1인실 vs 2인실 vs 4인실 — 책상 1200×700mm 고정모듈 기준 가구배치 비교</text>`);
...
s.push(`<rect x="${ox+0.15*S}" y="${oy+0.1*S}" width="${1.59*S}" height="${0.45*S}" fill="#eef7f0" stroke="#9cc9a8" stroke-dasharray="4,3"/>`);
s.push(`<text x="${ox+1.89*S/2}" y="${oy+0.1*S+18}" font-size="9" fill="#16834a" text-anchor="middle">여유공간(통행)</text>`);
s.push(chair(0.945,0.85,ox,oy));
s.push(desk(0.345,1.05,1.2,0.7,ox,oy));
...
s.push(desk(0.1,0.5,0.7,1.2,ox,oy));
s.push(chair(0.95,1.1,ox,oy));
s.push(desk(0.1,1.7,0.7,1.2,ox,oy));
s.push(chair(0.95,2.3,ox,oy));
s.push(`<rect x="${ox+1.2*S}" y="${oy+0.5*S}" width="${0.95*S}" height="${2.9*S}" fill="#f0f9ff" stroke="#bcd9ef" stroke-dasharray="4,3"/>`);
s.push(`<text x="${ox+1.675*S}" y="${oy+1.95*S}" font-size="8.5" fill="#1c6ea4" text-anchor="middle" transform="rotate(-90 ${ox+1.675*S} ${oy+1.95*S})">오른쪽 통로(안쪽 책상도 앞 책상 안 지나고 바로 진입)</text>`);
...
s.push(desk(0.1,0.45,0.7,1.2,ox,oy));
s.push(chair(0.95,1.05,ox,oy));
s.push(desk(0.1,1.7,0.7,1.2,ox,oy));
s.push(chair(0.95,2.3,ox,oy));
s.push(desk(2.64,0.45,0.7,1.2,ox,oy));
s.push(chair(2.49,1.05,ox,oy));
s.push(desk(2.64,1.7,0.7,1.2,ox,oy));
s.push(chair(2.49,2.3,ox,oy));
s.push(`<rect x="${ox+1.35*S}" y="${oy+0.1*S}" width="${0.74*S}" height="${2.7*S}" fill="#fff7ed" stroke="#f0d39d" stroke-dasharray="4,3"/>`);
