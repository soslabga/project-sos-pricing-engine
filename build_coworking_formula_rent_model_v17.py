...
# 방구성/전용평수/CAPEX는 각 실매물 배치도 기준 고정값. 가격(4/2/1인실가·월세)은 00_규모모델(분당표준_판교파일럿)에서 지역 티어 드롭다운으로 재계산됨
...
def add_layout(wb):
    # 100/120/150평형 실제 배치도(SVG→PNG 렌더, 지역명·가격 텍스트 제거된 중립본). 방구성은 00_규모모델(분당표준_판교파일럿) SIZE_MODELS와 정확히 일치(2026-07-10 재검증).
    ws = wb.create_sheet("00_배치도", 1)
    ws["A1"] = "규모별(100·120·150평) 배치도 — 실제 배치도 기준(방구성은 00_규모모델(분당표준_판교파일럿)과 동일)"
...
def add_common_assumptions(wb):
    # 공통 가정(정책 상수)은 별도 탭으로 분리 — 00_규모모델(분당표준_판교파일럿) 맨 앞은 큰 그림(구성→매출→비용→손익)만 보이게
    ws = wb.create_sheet("00_공통가정", 2)
    ws.append(["공통 가정 — 3개 규모 모델(100·120·150평)에 동일 적용되는 정책 상수. 00_규모모델(분당표준_판교파일럿) 시트 수식이 이 값들을 참조함"])  # row1
    ws.append(["항목", "값", "근거"])  # row2
    ws.append(["관리비 단가(만원/평)", 1.5, "실측 706호1.18만/평·1103호1.36만/평(비용_항목별_검증표.html, 2026-07-01) 대비 보수적 상향값"])  # row3
    ws.append(["전기 단가(만원/평)", 0.6, "실측 1103호 0.49만/평(비용_항목별_검증표.html) 대비 보수적 상향값"])  # row4
...
def add_size_model(wb):
    ws = wb.create_sheet("00_규모모델(분당표준_판교파일럿)", 0)
    cols = ["B", "C", "D"]
    CA = "'00_공통가정'!"
    ...
    add_row("보증금(만원, 고정)", [7500, 7500, 10000])  # row41 — 실계약 보증금(월세와 별개, 반환의무 부채). 100/150평은 mjs 자체 출처주석으로 확인, 120평은 파일값은 있으나 독립 출처 인용 없음(비고 참조)
    add_row("모델 성격(비고)", ["전국 확장 표준모델", "판교 파일럿 참고 케이스 — 유치업종 제한 확인된 예외 입지, 전국 표준 아님", "판교 파일럿 참고 케이스 — 유치업종 제한 확인된 예외 입지, 전국 표준 아님"])  # row42
    ws.append(["출처"] + [m[13] for m in SIZE_MODELS])          # row43
    ws.append(["※ 방구성(4/2/1인실 수)·전용평수·초기투자비는 각 평형의 실제 배치도 기준 고정값이며, 지역 티어(B2)를 바꾸면 전용평당월세·객실가·월세(row10~14)만 재계산됩니다. 지역 티어(A~E) 자체는 미확보 매물 지역 참고용 가정. 임대차 계약기간·렌트프리 조건은 원본 3개 mjs 파일 어디에도 없어 미확보(협상 미확정 사안으로 추정)."])  # row44
    ...
    notes = {
        ...
        22: "인테리어·가구·네트워크·설계 등 초기투자비 — 평수·호실·좌석 규모에 비례해 산정. 전기통신·도어락·보안·냉난방·소방 항목은 실제 견적(비용_항목별_검증표.html)으로 확인됨. 인테리어 단가는 아직 견적 전 가정치(미확보)",
        ...
        41: "100평형(7,500)·150평형(10,000)은 각 mjs 파일 자체 출처주석(확인매물+날짜)으로 확인됨. 120평형(7,500)은 파일에 값은 있으나 100평형과 동일 수치를 재사용한 것으로 보이며 독립된 확인매물 인용이 없음 — 별도 확인 필요",
        42: "100평형(분당)은 전국 어디든 적용 가능한 표준 모델. 120·150평형(판교)은 판교 1103호라는 특정 입지의 파일럿 사례 — 해당 입지는 유치업종 제한이 확인된 예외 입지라 전국 확장 표준으로 일반화하면 안 됨",
...
def add_summary(wb):
    ...
    ws.append(["요약"])
    ws.append(["항목", "내용", "비고"])
    ws.append(["모델", "100·120·150평 임차형 소형특화 공유오피스 (1·2·4인실 중심, 지정석 제외)", "실매물 확정 3개 모델 — 00_규모모델(분당표준_판교파일럿) 시트 참조"])
    ws.append([])
    ws.append(["규모별 월 손익 — 한눈에 보기 (00_규모모델(분당표준_판교파일럿) 원자료, 수식 연동, 지역 티어는 00_규모모델(분당표준_판교파일럿)!B2 선택값 기준)"])
    ws.append(["구분"] + [m[0] for m in SIZE_MODELS])
    ws.append(["='00_규모모델(분당표준_판교파일럿)'!A29", "='00_규모모델(분당표준_판교파일럿)'!B29", "='00_규모모델(분당표준_판교파일럿)'!C29", "='00_규모모델(분당표준_판교파일럿)'!D29"])
    ws.append(["='00_규모모델(분당표준_판교파일럿)'!A30", "='00_규모모델(분당표준_판교파일럿)'!B30", "='00_규모모델(분당표준_판교파일럿)'!C30", "='00_규모모델(분당표준_판교파일럿)'!D30"])
    ws.append(["='00_규모모델(분당표준_판교파일럿)'!A31", "='00_규모모델(분당표준_판교파일럿)'!B31", "='00_규모모델(분당표준_판교파일럿)'!C31", "='00_규모모델(분당표준_판교파일럿)'!D31"])
    ws.append(["='00_규모모델(분당표준_판교파일럿)'!A32", "='00_규모모델(분당표준_판교파일럿)'!B32", "='00_규모모델(분당표준_판교파일럿)'!C32", "='00_규모모델(분당표준_판교파일럿)'!D32"])
    ws.append([])
    ws.append(["='00_규모모델(분당표준_판교파일럿)'!A35", "='00_규모모델(분당표준_판교파일럿)'!B35", "='00_규모모델(분당표준_판교파일럿)'!C35", "='00_규모모델(분당표준_판교파일럿)'!D35"])
    ws.append(["='00_규모모델(분당표준_판교파일럿)'!A36", "='00_규모모델(분당표준_판교파일럿)'!B36", "='00_규모모델(분당표준_판교파일럿)'!C36", "='00_규모모델(분당표준_판교파일럿)'!D36"])
    ws.append(["='00_규모모델(분당표준_판교파일럿)'!A37", "='00_규모모델(분당표준_판교파일럿)'!B37", "='00_규모모델(분당표준_판교파일럿)'!C37", "='00_규모모델(분당표준_판교파일럿)'!D37"])
    ws.append(["='00_규모모델(분당표준_판교파일럿)'!A38", "='00_규모모델(분당표준_판교파일럿)'!B38", "='00_규모모델(분당표준_판교파일럿)'!C38", "='00_규모모델(분당표준_판교파일럿)'!D38"])
    ws.append([])
    ws.append(["참고: BEP(손익분기 가동률)", "='00_규모모델(분당표준_판교파일럿)'!B26", "='00_규모모델(분당표준_판교파일럿)'!C26", "='00_규모모델(분당표준_판교파일럿)'!D26"])  # row17
    ws.append([])
...
def add_competitor(wb):
    ...
    P1, P2, P4 = "='00_규모모델(분당표준_판교파일럿)'!B14*10000", "='00_규모모델(분당표준_판교파일럿)'!B13*10000", "='00_규모모델(분당표준_판교파일럿)'!B12*10000"
...
def add_investment_recovery(wb):
    ...
    ws.append([])  # row23
    ws.append(['="결론 — 분당(전국 표준모델) 총소요자금 기준 회수기간 "&TEXT(B20,"0.0")&"개월. 판교(파일럿) 120/150평형은 유치업종 제한이 확인된 예외 입지 참고치이며 전국 확장 판단에는 사용하지 않음"'])  # row24
    ...
    notes = {
        ...
        8: "월매출 × 변동비율",
        9: "00_규모모델(분당표준_판교파일럿)의 월 고정비 합계 그대로 가져옴",
        10: "월매출 - 변동비 - 월고정비",
        11: "초기투자비 월상각은 실제 현금 유출이 아니므로(감가상각) 영업손익에 다시 더해 현금흐름으로 환산",
        12: "영업손익 + 초기투자비 월상각(비현금 비용 환입) = 실제 월 현금창출액",
        14: "00_규모모델(분당표준_판교파일럿)의 초기투자비(CAPEX) 그대로 가져옴",
        15: "실계약 보증금 — 100평형·150평형은 mjs 원본 확인, 120평형은 100평형과 동일값 재사용 추정(독립 출처 없음, 00_규모모델(분당표준_판교파일럿) 41행 비고 참조)",
        16: "pangyo_120.mjs/pangyo_150.mjs 원본의 총소요자금 계산식(CAPEX+보증금+운전자금500만) 근거",
