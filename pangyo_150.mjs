...
const py=150, rent=560, mgmtRate=2.8, partTime=120, sales=50; // 정상화 이후(steady-state), 소상공인 평균(20~100만) 상단. 런칭기(1~6개월)는 200만 별도 민감도 참고
...
console.log(`고정비 ${fk(fixed)}만 = 월세${rent}+관리비${mgmt}+전기${elec}+청소${clean}+운영잡비${opexFlat}+파트타임${partTime}+세일즈${sales}+상각${dep}`);
