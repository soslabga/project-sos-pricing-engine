...
export async function sendReservationSms(booking) {
  ...
  const failed = Number(result?.groupInfo?.count?.registeredFailed || 0);
  if (failed > 0) {
    const reason = result?.failedMessageList?.[0]?.statusMessage || "SMS_SEND_FAILED";
...
export async function sendLookupVerificationSms({ phone, code }) {
  ...
  const failed = Number(result?.groupInfo?.count?.registeredFailed || 0);
  if (failed > 0) throw new Error(result?.failedMessageList?.[0]?.statusMessage || "SMS_SEND_FAILED");
  return { mode: "solapi", success: true, messageId: result?.messageList?.[0]?.messageId || result?.groupInfo?.groupId };
}
