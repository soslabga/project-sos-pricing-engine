...
s.push(`<text x="160" y="40" font-size="18.5" font-weight="800" fill="#0f1e3d">판교1 v2 — 전용 100평 · 회의실 6/4/2인 · 줄4 2인+1인</text>`);
s.push(`<text x="160" y="60" font-size="11" fill="#64748b">줄1: 4인 3,670×2,400(풀폭) · 줄2~3: 4인 3,440×2,900 · 줄4: 2인 2,200×2,900 + 1인 1,850×1,400×4개 · 복도1,100</text>`);
...
// 줄4: 6×2인 + 1인 4개(2열×2행)
x=0.12;
for(let i=0;i<6;i++){room(x,11.02,2.2,2.9,2,'#dce8ff','2200×2900');x+=2.3;}
const x1=x, x2=x+1.99; // 1890×1400=0.80평
room(x1,11.02,1.89,1.4,1,'#ede8ff','1890×1400');
room(x2,11.02,1.89,1.4,1,'#ede8ff','1890×1400');
room(x1,12.52,1.89,1.4,1,'#ede8ff','1890×1400');
room(x2,12.52,1.89,1.4,1,'#ede8ff','1890×1400');

cor(0,14.02,SPINE,0.85,'복도 1,100 (공용 진입)');

// 공용 (y=14.87, h=2.48) — 회의실 2인 추가, 라운지 5.0m로 축소
// 6인(3.4) + 4인(2.9) + 2인(1.8) + 탕비(2.3) + 우편(1.9) + 라운지(4.95) = 17.25m
...
cm(6.55,1.8,'회의실 2인','#e8f4fb','1800×2480');   // ← 신규
cm(8.45,2.3,'탕비·OA','#f1f5f9','2300×2480');
cm(10.85,1.9,'우편·소포','#fff7ed','1900×2480');
cm(12.85,4.95,'라운지 + 리셉션 (입구 동선)','#f0fdf4','4950×2480');

s.push(`<text x="${X(9.3)}" y="${OY+FH*S+40}" font-size="13" font-weight="800" fill="#15803d" text-anchor="middle">독립실 ${rm}호실 / ${seat}석 (4인 ${mix[4]}·2인 ${mix[2]}·1인 ${mix[1]}) + 회의실 6/4/2인 + 라운지·리셉션 + 탕비</text>`);
...
fs.writeFileSync('C:/Users/User/Documents/프로젝트/코워킹_평면도_판교1_v2_2인미팅.svg',s.join('\n')+'</svg>','utf8');
...
console.log(`판교1 v2: ${rm}호실 ${seat}석 (4인${mix[4]} 2인${mix[2]} 1인${mix[1]})`);
