...
s.push(`<text x="160" y="60" font-size="11" fill="#64748b">분당 수내역 · 전용 120평 · 월세 432만·보증금 7,500만 · 4인128/2인75/1인45만 · 복도1,200</text>`);
...
s.push(`<rect x="${X(FW)-10}" y="${Y(15.5)}" width="20" height="${2.0*S}" fill="#1d4ed8" stroke="#fff" stroke-width="2"/>`);
s.push(`<polygon points="${X(FW)+22},${Y(16.5)} ${X(FW)+2},${Y(16.3)} ${X(FW)+2},${Y(16.7)}" fill="#1d4ed8"/>`);
s.push(`<rect x="${X(FW)+24}" y="${Y(16.5)-14}" width="46" height="28" rx="4" fill="#1d4ed8"/>`);
s.push(`<text x="${X(FW)+47}" y="${Y(16.5)+5}" font-size="14" font-weight="900" fill="#fff" text-anchor="middle">입구</text>`);
...
// 줄2: 6열 4인 (w=3.38, step=3.48, SPINE=20.9) — 문은 복도1(위) 방향
...
// 공용 (1인실 확장분 0.7m 빌려옴, 깊이 2.58→2.28)
const cm=(x,w,n,fl,dim)=>{
  s.push(`<rect x="${X(x)}" y="${Y(15.57)}" width="${w*S}" height="${2.28*S}" fill="${fl}" stroke="#1e293b" stroke-width="1.3"/>`);
  s.push(`<text x="${X(x+w/2)}" y="${Y(16.71)}" font-size="12" font-weight="700" fill="#0f1e3d" text-anchor="middle">${n}</text>`);
  if(dim)s.push(`<text x="${X(x+w/2)}" y="${Y(16.71)+12}" font-size="8.5" fill="#64748b" text-anchor="middle">${dim}</text>`);
};
// 우편·소포·창고는 줄4에 별도 룸으로 이미 배치됨(공용부 중복 배치 안 함)
cm(0.05,3.4,'회의실 6인','#eef3fb','3400×2280');
cm(3.55,2.9,'회의실 4인','#eef3fb','2900×2280');
cm(6.55,1.8,'회의실 2인','#e8f4fb','1800×2280');
cm(8.45,2.3,'OA','#f1f5f9','2300×2280');
cm(10.85,10.0,'라운지 + 탕비 + 리셉션 (입구 동선)','#f0fdf4','10000×2280');
...
// [가격정책 최종확정 2026-07-01] 하이브리드(pangyo11.mjs 방법론). 분당=SP파격가(대형실균일33만/석) 대비 -9%=30만/석 하한 검증(1인40/2인60/4인120), cost-plus(28.7만/석)보다 높아 경쟁사밴드 채택
...
const capex=Math.round((py*130+py*4+py*20+py*9+rm*7+86+seat*23.65+50+150)*1.05); // [라운지가구 100→150만 상향 2026-07-02, 실시세 검토] [원복 2026-07-01] 소방·냉난방은 이미지에 명시된 "소계" 합계로 재확정: 소방설비공사 소계=12,369,010+14,920,660=27,289,670원(소방4층.png 화면의 소계란 직접 확인)÷301.54평=9.05만/평. 냉난방 장비33,263,000+설치26,130,000=59,393,000원(냉난방.png+냉난방2.png 항목별 전체합산)÷301.54평=19.70만/평. 무선AP(Cisco1,999만)는 SOS 본사용 과사양이라 제외(검증표 판단 유지) + 라운지가구100만 + 전기통신7만/평 + 도어락7만/룸 + 보안200만
...
console.log(`[런칭기 참고, 1~6개월] 정액 마케팅비 +${launchMkt}만 가정 시: 고정비${fk(fixedLaunch)}만 · BEP ${bepLaunch.toFixed(1)}% (정상 BEP 아님, 초기 한시적. 이후 매출7% 변동비로 자연 감소)`);console.log(`총 소요자금 ${fk(totalCash)}만 (CAPEX+보증금${depo}+운전500)`);
