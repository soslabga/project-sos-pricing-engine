...
s.push(`<text x="160" y="60" font-size="11" fill="#64748b">분당 수내역 · 전용 105평(계약121평) · 월세 432만·보증금 7,500만 · 4인124/2인70/1인40만 · 복도1,100</text>`);
...
// 줄4: 7×2인 + 1인 4개(2열×2행)
x=0.12;
for(let i=0;i<7;i++){room(x,11.02,2.2,2.9,2,'#dce8ff','2200×2900');x+=2.3;}
// 1인 섹션: x=16.22~20.9 = 4.68m → 2열(각 2.29m)
const i1x1=x, i1x2=x+1.99; // 1890×1400=0.80평
room(i1x1,11.02,1.89,1.4,1,'#ede8ff','1890×1400');
room(i1x2,11.02,1.89,1.4,1,'#ede8ff','1890×1400');
room(i1x1,12.52,1.89,1.4,1,'#ede8ff','1890×1400');
room(i1x2,12.52,1.89,1.4,1,'#ede8ff','1890×1400');
...
// 0.05+3.4+0.1+2.9+0.1+1.8+0.1+2.3+0.1+1.9+0.1+8.05 = 20.85 ≈ SPINE 20.9
cm(0.05,3.4,'회의실 6인','#eef3fb','3400×2980');
cm(3.55,2.9,'회의실 4인','#eef3fb','2900×2980');
cm(6.55,1.8,'회의실 2인','#e8f4fb','1800×2980');
cm(8.45,2.3,'탕비·OA','#f1f5f9','2300×2980');
cm(10.85,1.9,'우편·소포','#fff7ed','1900×2980');
cm(12.85,8.0,'라운지 + 리셉션 (입구 동선)','#f0fdf4','8000×2980');
...
const full_rev=mix[4]*124+mix[2]*70+mix[1]*40;
